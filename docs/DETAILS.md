...
- **`omo@sisyphuslabs` 의 원본은 캐시만 봐서는 알 수 없다.** Codex 마켓플레이스가 전부 로컬 경로라
  `~/.codex/plugins/cache/sisyphuslabs` 에는 원본 URL 이 없다. 실제 출처는 플러그인 폴더 안의
  `lazycodex-install.json` 에 적힌 npm 패키지 `lazycodex-ai`
  ([github.com/code-yeongyu/lazycodex](https://github.com/code-yeongyu/lazycodex)) 다.
  `npx lazycodex-ai install` 이 내부적으로
  `npx --yes --package oh-my-openagent omo install --platform=codex` 를 부르고,
  플러그인은 `lazycodex` 가 아니라 `omo` 라는 이름으로 등록된다. 이름이 달라서 연결이 안 보인다.
  LazyCodex 는 배포판이고 엔진인 OmO(`code-yeongyu/oh-my-openagent`)를 서브모듈로 담고 있다.
  `install-codex.sh` 가 미설치를 감지하면 확인을 받고 이 명령으로 설치한다.
